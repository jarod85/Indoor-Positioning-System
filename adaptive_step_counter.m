function [distance, max_locs] = adaptive_step_counter(filename)
    load(filename); 
    ...
    for i = 1:length(max_pks)
        relative_accel = alpha*max_pks(i)/avg_accel;
        if (i > length(step_duration))
            relative_step_duration = avg_step_duration;
        else
            relative_step_duration = beta*step_duration(i)/avg_step_duration;
        end
        % average out 2 factors to get their overall impact on step length
        displacement(i) = ((relative_accel + relative_step_duration)/2)*avg_step_len + shoe_length;
        if (i == 1)
